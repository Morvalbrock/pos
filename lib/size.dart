...
import 'package:pos/addsize.dart';
import 'package:pos/editing_size.dart';
import 'package:pos/view_size.dart';
...
class _MyAppState extends State<Sizepage> {
  ...
  Future<dynamic> getOrder() async {
    ...
    List<dynamic> rs = await SelectionQry(
        'SELECT * FROM  tbl_size  WHERE comp_id = "$comp_id"');
    product.clear();
    for (var item in rs) {
      // print(item['orderID']);
                              ...
                              child: SfDataGrid(
                                  ...
                                  onCellTap: (details) {
                                    ...
                                    Map<String, dynamic> fetchedData = {
                                      ...
                                      'sz_id': row.getCells()[2].value,
                                      'sz_name': row.getCells()[3].value,
                                      'sz_short': row.getCells()[4].value,
                                      'sz_status': row.getCells()[5].value,
                                      'created_at': row.getCells()[6].value,
                                      'update_at': row.getCells()[7].value,
                                    };
                                    showDialog(
                                        context: context,
                                        builder:
                                                                          ...
                                                                          children: [
                                                                            ElevatedButton(
                                                                                ...
                                                                                onPressed: () async {
                                                                                  Navigator.pop(context);
                                                                                  EasyLoading.show(status: 'loading...');
                                                                                  dynamic rs = await deletequery('DELETE FROM `tbl_size` WHERE `id` =' + fetchedData["id"].toString());
...
class OrderInfo {
  ...
  final String? sz_short;
  final String? sz_status;
  ...
  void buildPaginatedDataGridRows() {
    dataGridRows = paginatedDataSource.map<DataGridRow>((dataGridRow) {
      return DataGridRow(cells: [
        ...
        DataGridCell(columnName: 'sz_id', value: dataGridRow.sz_id),
        DataGridCell(columnName: 'sz_name', value: dataGridRow.sz_name),
        DataGridCell(columnName: 'sz_short', value: dataGridRow.sz_short),
        DataGridCell(columnName: 'sz_status', value: dataGridRow.sz_status),
