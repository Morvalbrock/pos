...
class _MyAppState extends State<StockReportpage> {
  ...
  Future<dynamic> getOrder() async {
    ...
    List<dynamic> rs = await SelectionQry(
        'SELECT *,(SELECT sup_name FROM `tbl_supplier` WHERE comp_id = tbl_stock.comp_id AND sup_id =tbl_stock.sup_id) AS sup_name,@a:=@a+1 serial_number FROM  tbl_stock,(SELECT @a:= 0) AS a WHERE comp_id = "$comp_id"');
    product.clear();
    for (var item in rs) {
      // print(item['orderID']);
      OrderInfo lst = OrderInfo(
        (item['serial_number']).round(),
        item['id'],
        item['comp_id'],
        item['stk_id'],
        item['pur_id'],
  ...
  void buildPaginatedDataGridRows() {
    dataGridRows = paginatedDataSource.map<DataGridRow>((dataGridRow) {
      return DataGridRow(cells: [
        DataGridCell(columnName: 'id', value: dataGridRow.serial_number),
