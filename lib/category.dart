...
import 'package:pos/addcatogary.dart';
import 'package:pos/edit_category.dart';
import 'package:pos/view_category.dart';
...
class _MyAppState extends State<Categorypage> {
  ...
  Future<dynamic> getOrder() async {
    ...
    List<dynamic> rs = await SelectionQry(
        'SELECT * FROM  tbl_category  WHERE comp_id = "$comp_id"');
    product.clear();
    for (var item in rs) {
      // print(item['orderID']);
                              ...
                              child: SfDataGrid(
                                  ...
                                  onCellTap: (details) {
                                    ...
                                    Map<String, dynamic> fetchedData = {
                                      ...
                                      'c_name': row.getCells()[3].value,
                                      'c_short': row.getCells()[4].value,
                                      'c_status': row.getCells()[5].value,
                                                                          ...
                                                                          children: [
                                                                            ElevatedButton(
                                                                                ...
                                                                                onPressed: () async {
                                                                                  Navigator.pop(context);
                                                                                  EasyLoading.show(status: 'loading...');
                                                                                  dynamic rs = await deletequery('DELETE FROM `tbl_category` WHERE `id` =' + fetchedData["id"].toString());
  ...
  void buildPaginatedDataGridRows() {
    dataGridRows = paginatedDataSource.map<DataGridRow>((dataGridRow) {
      return DataGridRow(cells: [
        ...
        DataGridCell(columnName: 'c_short', value: dataGridRow.c_short),
        DataGridCell(columnName: 'c_status', value: dataGridRow.c_status),
