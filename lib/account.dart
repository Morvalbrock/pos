...
import 'package:pos/addaccount.dart';
import 'package:pos/edit_account.dart';
import 'package:pos/view_account.dart';
...
class _MyAppState extends State<Accountpage> {
  ...
  Future<dynamic> getOrder() async {
    ...
    List<dynamic> rs = await SelectionQry(
        'SELECT * FROM  tbl_account  WHERE comp_id = "$comp_id"');
    product.clear();
    for (var item in rs) {
      // print(item['orderID']);
                              ...
                              child: SfDataGrid(
                                  ...
                                  onCellTap: (details) {
                                    ...
                                    Map<String, dynamic> fetchedData = {
                                      ...
                                      'acc_id': row.getCells()[2].value,
                                      'acc_root': row.getCells()[3].value,
                                      'acc_name': row.getCells()[4].value,
                                      'acc_status': row.getCells()[5].value,
                                                                          ...
                                                                          children: [
                                                                            ElevatedButton(
                                                                                ...
                                                                                onPressed: () async {
                                                                                  Navigator.pop(context);
                                                                                  EasyLoading.show(status: 'loading...');
                                                                                  dynamic rs = await deletequery('DELETE FROM `tbl_account` WHERE `id` =' + fetchedData["id"].toString());
